...
const fetchPoem = async (poet) => {
  ...
  const fetch = new Promise((resolved, rejected) => {
    osmosis
      .get(`http://so.gushiwen.org/author_${poet.poeId}.aspx`)
      .if('.right !~ .title')
      .then(() => {
        osmosis
          .get(`http://so.gushiwen.org//authors/authorsw_${poet.poeId}A1.aspx`)
          ...
          .data((listing) => {
            ...
            // 关联作者-诗人
            listing.poet = poet._id; // eslint-disable-line
          })
          .then((ctx, item) => {
            // console.log(item);
